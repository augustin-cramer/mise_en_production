...
def project_variation_on_axis(axis, source:str, year:int, df, number_of_words, with_parliament) :
    ...
    colors = ['#1f77b4', '#ff7f0e']  # Example: blue for 'tech' or 'pos', orange for 'reg' or 'neg'

    # Plotting with enhanced aesthetics
    for i, (data, label) in enumerate([(data_for_var_up, 'Tech' if axis == 1 else 'Positive'), 
                                    (data_for_var_down, 'Reg' if axis == 1 else 'Negative')]):
        axs[i].bar(data.keys(), data.values(), color=colors[i], label=f'Variation in {label}', alpha=0.75, edgecolor='black')
        axs[i].set_title(f'Variation in {label}', fontsize=14)
        axs[i].tick_params(axis='x', rotation=45)
        axs[i].tick_params(axis='both', which='major', labelsize=12)  # Bigger fonts for readability
    ...
    fig.suptitle(f"{number_of_words} words most responsible for the move of {source} towards the respective poles between year {year} and {year + 1}; axis = {axis}", fontsize=16)

    # Use tight_layout to automatically adjust subplot params. `rect` argument ensures suptitle is not overlapped.
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])

    # Save the figure
    plt.savefig(f"plots/Word_analysis/the {number_of_words} words most responsible for the move of {source} towards the respective poles between year {year} and {year + 1} ; axis = {axis}, with_parliament = {with_parliament}.png", dpi=300)  # dpi parameter to improve resolution
...
def axis_variation(axis, source=None, year=2013, number_of_words=30, with_parliament=True):
    ...
    if os.path.exists(f"plots/Word_analysis/the {number_of_words} words most responsible for the move of {source} towards the respective poles between year {year-1} and {year} ; axis = {axis}, with_parliament = {with_parliament}.png"):
        print('already computed')
        img = mpimg.imread(f"plots/Word_analysis/the {number_of_words} words most responsible for the move of {source} towards the respective poles between year {year-1} and {year} ; axis = {axis}, with_parliament = {with_parliament}.png")
