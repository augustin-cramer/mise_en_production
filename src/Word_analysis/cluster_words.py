...
df_BT_p = pd.read_csv('data/with parliament/current_dataframes/df_BT.csv', index_col=[0]).reset_index()
df_BT_wp = pd.read_csv('data/without parliament/current_dataframes/df_BT.csv', index_col=[0]).reset_index()
...
def cluster_words(year, axis, left_threshold=None, right_threshold=None, head=None, tail=None, with_parliament=True, percentiles=None, company = None):
    ...
    if percentiles:
        ...
        df_t = df_t.loc[(df_t[f'cos axe {axis}'] < quantiles[0]) | (df_t[f'cos axe {axis}'] > quantiles[1])]
