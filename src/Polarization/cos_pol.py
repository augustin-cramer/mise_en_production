...
def draw_cos_pol(
    ...
    if os.path.exists(
        f"plots/Polarization/Polarization and cosine between {left_side} VS {right_side} ; axis = {axis}, companies = {companies}, percentiles = {percentiles}, with parliament = {with_parliament}.png"
    ):
        img = mpimg.imread(
            f"plots/Polarization/Polarization and cosine between {left_side} VS {right_side} ; axis = {axis}, companies = {companies}, percentiles = {percentiles}, with parliament = {with_parliament}.png"
        )
        ...
        ax1.fill_between(
            x, CI_lows_real, CI_high_real, color="blue", alpha=0.05
        )
        ...
        plt.title(
            f"Polarization and cosine between {left_side} VS {right_side} ; axis = {axis}, companies = {companies}, percentiles = {percentiles}, with parliament = {with_parliament}"
        )
        plt.grid(
            True, linestyle="--", alpha=0.5, axis="x"
        )  # Adding both x and y gridlines
        plt.tight_layout()
        plt.savefig(
            f"plots/Polarization/Polarization and cosine between {left_side} VS {right_side} ; axis = {axis}, companies = {companies}, percentiles = {percentiles}, with parliament = {with_parliament}.png"
        )
