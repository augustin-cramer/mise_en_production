...
def draw_cos_pol(
    ...
    fig.add_trace(go.Scatter(x=x, y=real_pol, name='Polarisation réelle', line=dict(color='blue', width=2)))
    fig.add_trace(go.Scatter(x=x, y=CI_high_real, line=dict(width=0), showlegend=False, fill=None, mode='lines'))
    fig.add_trace(go.Scatter(x=x, y=CI_lows_real, fill='tonexty', fillcolor='rgba(0, 0, 255, 0.1)', line=dict(width=0), showlegend=False))

    # Add second set of data (cosine similarity of left side) to the secondary y-axis
    fig.add_trace(go.Scatter(x=x, y=Con_cos, name=f'Cosine similarity of {left_side}', line=dict(color='green', dash='dash', width=2), yaxis='y2'))
    fig.add_trace(go.Scatter(x=x, y=Con_CI_high, line=dict(width=0), showlegend=False, fill=None, mode='lines', yaxis='y2'))
    fig.add_trace(go.Scatter(x=x, y=Con_CI_low, fill='tonexty', fillcolor='rgba(0, 128, 0, 0.1)', line=dict(width=0), showlegend=False, yaxis='y2'))

    # Add third set of data (cosine similarity of right side) to the secondary y-axis
    fig.add_trace(go.Scatter(x=x, y=Lab_cos, name=f'Cosine similarity of {right_side}', line=dict(color='red', dash='dash', width=2), yaxis='y2'))
    fig.add_trace(go.Scatter(x=x, y=Lab_CI_high, line=dict(width=0), showlegend=False, fill=None, mode='lines', yaxis='y2'))
    fig.add_trace(go.Scatter(x=x, y=Lab_CI_low, fill='tonexty', fillcolor='rgba(255, 0, 0, 0.1)', line=dict(width=0), showlegend=False, yaxis='y2'))
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title='Cosine similarity', side='right', overlaying='y', rangemode='tozero', showgrid=False),
    ...
    for year in x:
        fig.add_vline(x=year, line=dict(color='gray', dash='dash', width=1), line_width=0.5)
